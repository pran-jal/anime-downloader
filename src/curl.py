...
class curl_downloader:
    def __init__(self, epi_url, epi_name, dir_name, *args):
        self.cmd = 'curl.exe "' + epi_url + headers + " -o " + epi_name + '.mp4'
    ...
    def track(self):
        ...
        while True:
            if self.download.stdout is None:
                continue
            line = self.download.stdout.read(79).decode("utf-8") # this 79 is trial and error guess dont know how and why this is working. Have to find an alternative.
            if line == "" and self.download.poll() is not None:
                break
            else:
                spl_line = line.split()
        ...
        while True:
            if self.download.stdout is None:
                continue
            line = self.download.stdout.read(79).decode("utf-8") # this 79 is a trial and error guess dont know who and why this is working. Have to find an altenative
            if line == "" and self.download.poll() is not None:
                break
            else:
                line = line.split()
